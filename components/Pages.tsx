    ...
    <PageWrapper className="!p-0">
        <div className="w-full h-full flex">
            ...
            <div className="w-2/3 bg-gray-100 text-gray-800 p-12 flex flex-col justify-center">
                <AnimatedElement isActive={isActive}>
                    <h2 className="font-montserrat text-3xl font-bold text-gray-800 mb-6">Guaranteed Media Deliverables</h2>
                </AnimatedElement>
                <ul className="space-y-4">
                    <AnimatedElement isActive={isActive} delay="delay-200">
                        <li className="flex items-center text-xl"><CheckCircle className="w-6 h-6 mr-3 text-red-600 flex-shrink-0"/> Half Page Ad (Pre-event)</li>
                    </AnimatedElement>
                    <AnimatedElement isActive={isActive} delay="delay-[300ms]">
                        <li className="flex items-center text-xl"><CheckCircle className="w-6 h-6 mr-3 text-red-600 flex-shrink-0"/> Full Page Ad (Post-event)</li>
                    </AnimatedElement>
                    <AnimatedElement isActive={isActive} delay="delay-[400ms]">
                        <li className="flex items-center text-xl"><CheckCircle className="w-6 h-6 mr-3 text-gray-500 flex-shrink-0"/> TOI.com Post-Event Article</li>
                    </AnimatedElement>
                    <AnimatedElement isActive={isActive} delay="delay-[500ms]">
                        <li className="flex items-center text-xl"><CheckCircle className="w-6 h-6 mr-3 text-gray-500 flex-shrink-0"/> TOI.com Banner Ads (3 Lakh Impressions)</li>
                    </AnimatedElement>
                </ul>
            </div>
        </div>
...
export const Page14: React.FC<PageProps> = ({ isActive }) => {
    const budgetData = [
        { category: 'College level expenses (400 colleges)', logic: '400 × 5,000', amountInr: '20,00,000', amountLakhs: '20' },
        { category: 'District level Expenses (20 districts)', logic: '20 × 75,000', amountInr: '15,00,000', amountLakhs: '15' },
        { category: 'Final round expenses (Prize money)', logic: 'Fixed', amountInr: '5,00,000', amountLakhs: '5' },
        { category: 'Food, accommodation & transportation', logic: 'Fixed', amountInr: '3,00,000', amountLakhs: '3' },
        { category: 'Misc.', logic: 'Variable', amountInr: '7,00,000', amountLakhs: '7' },
    ];
    
    return (
        <PageWrapper>
             <AnimatedElement isActive={isActive}>
                <h1 className="font-montserrat text-5xl font-extrabold text-center mb-4">Investment & Partnership</h1>
                <p className="text-xl text-center text-gray-300 mb-10">Breakdown of the proposed 50 Lakhs budget.</p>
            </AnimatedElement>
             <div className="w-full max-w-5xl bg-white/5 p-8 rounded-lg border border-white/20 backdrop-blur-sm">
                {/* Header */}
                <div className="grid grid-cols-10 gap-4 font-bold text-lg border-b-2 border-white/50 pb-3 mb-3 text-left">
                    <div className="col-span-4">Category</div>
                    <div className="col-span-3">Logic/Calculation</div>
                    <div className="col-span-2 text-right">Amount (in ₹)</div>
                    <div className="col-span-1 text-right">Lakhs</div>
                </div>
                 {/* Rows */}
                {budgetData.map((item, index) => (
                     <AnimatedElement
                        ...
                        delay={`delay-[${200 + index * 100}ms]`}
                     >
                        <div className="grid grid-cols-10 gap-4 py-3 border-b border-white/10 items-center text-left">
                           <div className="col-span-4">{item.category}</div>
                           <div className="col-span-3 text-gray-300">{item.logic}</div>
                           <div className="col-span-2 text-right font-semibold">₹ {item.amountInr}</div>
                           <div className="col-span-1 text-right font-semibold">{item.amountLakhs}</div>
                        </div>
                    </AnimatedElement>
                ))}
                 {/* Footer */}
                <div className="grid grid-cols-10 gap-4 font-bold text-xl mt-4 text-left">
                    <div className="col-span-4">Total Amount</div>
                    <div className="col-span-3 text-gray-300">Sum of all categories</div>
                    <div className="col-span-2 text-right" style={{color: COLORS.FEDERAL_YELLOW}}>₹ 50,00,000</div>
                    <div className="col-span-1 text-right" style={{color: COLORS.FEDERAL_YELLOW}}>50</div>
                </div>
            </div>
             <AnimatedElement isActive={isActive} delay="delay-[800ms]">
                <p className="mt-10 text-center text-2xl max-w-4xl">
                    We invite Federal Bank to partner with us in this landmark initiative to build and nurture the next generation of India's leaders.
                </p>
            </AnimatedElement>
        </PageWrapper>
    );
...
export const Page15: React.FC<PageProps> = ({ isActive }) => (
            ...
            <AnimatedElement isActive={isActive} delay="delay-[400ms]" className="mb-10">
                <div className="bg-black/30 backdrop-blur-sm p-6 rounded-lg border border-white/20 inline-flex flex-col gap-4">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <a href="tel:+919544230903" className="font-inter text-2xl font-semibold tracking-wider text-white hover:underline transition-all">
                           +91 95442 30903
                        </a>
                    </div>
                </div>
